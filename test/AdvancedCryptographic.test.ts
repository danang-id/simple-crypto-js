...
const cipherTextFromString = firstInstance.setEncoder(SimpleCrypto.encoders.Utf16LE).update(string).encrypt()
const cipherTextFromNumber = firstInstance.setEncoder(SimpleCrypto.encoders.Default).update(number).encrypt()
const cipherTextFromBoolean = firstInstance.update(boolean).encrypt()

const cipherTextFromConcatObject = secondInstance.setSecret(secondSecretKey).update(object).append(object).encrypt()
const cipherTextFromConcatString = secondInstance
	.setSecret(secondSecretKey.toString())
	.update(string)
	.append(string)
	.encrypt()
const cipherTextFromConcatNumber = secondInstance.update(number).append(number).encrypt()
const cipherTextFromConcatBoolean = secondInstance.update(boolean).append(boolean).encrypt()

const plainExpectsObject = firstInstance.update(cipherTextFromObject).decrypt()
const plainExpectsString = firstInstance
	.setEncoder(SimpleCrypto.encoders.Utf16LE)
	.update(cipherTextFromString)
	.decrypt()
const plainExpectsNumber = firstInstance
	.setEncoder(SimpleCrypto.encoders.Default)
	.update(cipherTextFromNumber)
	.decrypt()
const plainExpectsBoolean = firstInstance.decrypt(cipherTextFromBoolean, false, SimpleCrypto.encoders.Utf8)
...
function halvesString(src: string): string[] {
	...
	return [src.substring(0, halfLength), src.substring(halfLength)]
}
...
describe("Advanced Cryptographic: Encryption", () => {
	it("should be able to encrypt from object using advanced method", () => {
		...
		expect(cipherTextFromConcatObject.substring(213, 214)).not.to.be.equals("=")
		expect(cipherTextFromConcatObject.substring(214, 216)).to.be.equals("==")
	})
	...
	it("should be able to encrypt from string using advanced method", () => {
		...
		expect(cipherTextFromConcatString.substring(126, 128)).not.to.be.equals("==")
	})
	...
	it("should be able to encrypt from number using advanced method", () => {
		...
		expect(cipherTextFromConcatNumber.substring(106, 107)).not.to.be.equals("=")
		expect(cipherTextFromConcatNumber.substring(107, 108)).to.be.equals("=")
	})
	...
	it("should be able to encrypt from boolean using advanced method", () => {
		...
		expect(cipherTextFromConcatBoolean.substring(85, 86)).not.to.be.equals("=")
		expect(cipherTextFromConcatBoolean.substring(86, 88)).to.be.equals("==")
	})
})
